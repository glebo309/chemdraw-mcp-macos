...
- `standard-scope-parent.json`: explicitly mapped acetophenone argument fixture for offline standard scope proposals. This is an illustrative software fixture, not experimental data. The CLI accepts these values through `--parent` and `--handle-map`, not by reading this file. Proposals contain no experimental results or yields.
