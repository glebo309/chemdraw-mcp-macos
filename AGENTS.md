...
Public GitHub hosting was approved on 2026-09-15 for glebo309/chemdraw-mcp-macos, superseding the initial private-only decision. Glenn subsequently approved open-source licensing; original project code is AGPL-3.0-only. Retain LICENSE, NOTICE and all upstream notices. This does not authorize package publication, contacting maintainers or adding collaborators. Keep private research, proprietary references, credentials and local validation bundles out of repository history. Subsequent pushes require corresponding user scope.
