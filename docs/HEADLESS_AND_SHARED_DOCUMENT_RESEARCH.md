...
Glenn requests truly headless native rendering if possible. If desktop ChemDraw
...
Glenn approved testing a shared-document clipboard bridge and deprioritized true
headless rendering. Disposable probes only; no production clipboard endpoint is
...
incorrect chemistry in an earlier completed trial. Glenn supplied the native
dialog screenshot during the second MOL trial and was told to cancel, not expand.
