...
Reviewed 2026-09-15 after Glenn pointed to his existing SN2 drawing. This records the original reference and wider proposal. The current README and usage guide describe the subsequently implemented bounded curve tool; new dots, native moving attachment and automatic routing remain proposals.

## Start from the working drawing

The local reference is `/Users/glenn/Downloads/SN2_house_style/`. Its `SN2_preview.png` was visually inspected. Both `SN2_example.cdxml` and `SN2_example-native.cdxml` contain the two curved electron-pair arrows. The generator is `/Users/glenn/ChemDraw-Output/horgh1-v9/sn2_example.py`; its `curve` function writes native CDXML curves, and its renderer uses AppleScript, not mouse automation.
