...
Use this checklist when preparing a release. Experimental builds are available
on [GitHub Releases](https://github.com/glebo309/chemdraw-mcp-macos/releases).
Stable releases additionally require independent-Mac acceptance and Developer ID
signing and notarization. Passing tests on one development Mac does not establish
compatibility with other macOS or ChemDraw versions.

## Required release gates

- [ ] Retain the AGPL-3.0-only LICENSE, NOTICE and all applicable dependency notices.
...
- [ ] Sign and notarize the Mac application before a stable release.
...
- [ ] Review all public documentation and release notes as product documentation. Exclude conversation transcripts, personal filesystem paths, internal task instructions and approval records. Check the documentation embedded in source archives as well as the repository.

## Acceptance procedure

For shared-document acceptance, start with [TEST_ON_MAC.md](TEST_ON_MAC.md).
...
Historical rc3 results are retained in [VALIDATION-0.10.0rc3.md](VALIDATION-0.10.0rc3.md).
...
Include the versioned installer, corresponding source, locked dependency metadata,
licence notices, reproducible examples and checksums. Release notes should describe
changes, tested environments and known limitations. Build artifacts must correspond
to the tagged source. Keep detailed local test outputs outside the distribution.
